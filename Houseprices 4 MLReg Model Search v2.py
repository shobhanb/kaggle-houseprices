...
parameters = [
    {
        'reg__estimator': [Ridge(), Lasso()],
        'reg__estimator__alpha': np.arange(0.25,2,0.25),
    },
    {
        'reg__estimator': [ElasticNet()],
        'reg__estimator__alpha': np.arange(0.25,2,0.25),
        'reg__estimator__l1_ratio': np.arange(0.0,1,0.25),
    },
    {
        'reg__estimator': [SGDRegressor()],
        'reg__estimator__alpha': [0.0001, 0.0003, 0.001, 0.003, 0.007, 0.01, 0.03],
        'reg__estimator__penalty': ['l1', 'l2', 'elasticnet'],
    },
    {
        'reg__estimator': [SVR()],
        'reg__estimator__kernel': ['linear', 'poly', 'rbf', 'sigmoid'],
        'reg__estimator__C': np.arange(0.25, 2, 0.25),
    },
    {
        'reg__estimator': [KNeighborsRegressor()],
        'reg__estimator__algorithm': ['brute'],
        'reg__estimator__n_neighbors': np.arange(3, 10, 2),
        'reg__estimator__weights': ['uniform', 'distance'],
        'reg__estimator__p': np.arange(1, 2, 0.25),
    },
    {
        'reg__estimator': [DecisionTreeRegressor()],
        'reg__estimator__criterion': ['mse', 'friedman_mse', 'mae'],
        'reg__estimator__max_depth': np.arange(1, 50, 5),
    },
]
...
gscv = GridSearchCV(model_pipeline, param_grid=parameters, scoring='neg_root_mean_squared_error', cv=5, n_jobs=-1, verbose=10)
...
filename = 'Submission 4 - Tuned SVR.csv'
